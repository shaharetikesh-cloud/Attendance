...
def dashboard(request):
    ...
    context = {
        ...
        'operator_count': Employee.objects.filter(substation__in=allowed_substations, employee_type=Employee.EmployeeType.OPERATOR).count(),
        'tech_count': Employee.objects.filter(substation__in=allowed_substations, employee_type=Employee.EmployeeType.TECH_ENGINEER).count(),
    ...
    if get_user_role(request.user) in APPROVER_ROLES:
        pending_count = 0
        for model in (OperatorAttendanceSheet, AdvanceShiftChart, TechAttendanceSheet, ApprenticeAttendanceSheet, OutsourceAttendanceSheet):
            pending_count += model.objects.filter(substation__in=allowed_substations, approval_status=MonthlySheetBase.STATUS_PENDING).count()
...
def signup_view(request):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            form.save()
            messages.success(request, 'Signup request submitted. Admin approval is required before login.')
            return redirect('login')
    else:
        form = SignupForm()
    return render(request, 'core/signup.html', {'form': form, 'self_signup_enabled': True})
...
def user_management(request):
    ...
    if request.GET.get('user_id'):
        selected_user = get_object_or_404(User.objects.select_related('profile'), pk=request.GET.get('user_id'))

    if request.method == 'POST':
        selected_user = get_object_or_404(User.objects.select_related('profile'), pk=request.POST.get('user_id'))
        ...
        if profile_form.is_valid() and access_form.is_valid():
            ...
            selected_ids = set(access_form.cleaned_data['substations'].values_list('id', flat=True))
            UserSubstationAccess.objects.filter(user=selected_user).exclude(substation_id__in=selected_ids).delete()
            existing_ids = set(UserSubstationAccess.objects.filter(user=selected_user).values_list('substation_id', flat=True))
    ...
    if selected_user and not profile_form:
        ...
        access_form = UserAccessForm(initial={'substations': UserSubstationAccess.objects.filter(user=selected_user).values_list('substation_id', flat=True)})

    users = User.objects.select_related('profile').order_by('username')
    return render(request, 'core/user_management.html', {'users': users, 'selected_user': selected_user, 'profile_form': profile_form, 'access_form': access_form})
...
def signup_requests(request):
    if request.method == 'POST':
        signup_request = get_object_or_404(SignupRequest.objects.select_related('user', 'user__profile'), pk=request.POST.get('request_id'))
        ...
        if form.is_valid():
            ...
            signup_request.status = SignupRequest.STATUS_APPROVED if action == 'approve' else SignupRequest.STATUS_REJECTED
            ...
            messages.success(request, f'Signup request {signup_request.get_status_display().lower()} successfully.')
    ...
    return render(request, 'core/signup_requests.html', {'requests': requests_qs, 'approval_form': form})
...
def system_settings(request):
    if request.method == 'POST':
        ...
        if form.is_valid():
            AppSetting.set_value('self_signup_enabled', 'true' if form.cleaned_data['self_signup_enabled'] else 'false')
            AppSetting.set_value('approval_required', 'true' if form.cleaned_data['approval_required'] else 'false')
            messages.success(request, 'System settings updated successfully.')
            return redirect('system_settings')
    else:
        form = SimpleSettingForm(initial={'self_signup_enabled': _self_signup_enabled(), 'approval_required': _approval_required()})
